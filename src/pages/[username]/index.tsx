...
const ProfileVersus: NextPageWithLayout<PageProps> = (props) => {
 return <Profile key={props.user.id} {...props} type="versus" />;
};

ProfileVersus.getLayout = (page) => <RootLayout>{page}</RootLayout>;
export default ProfileVersus;

export const getServerSideProps: GetServerSideProps = async ({ params, req, res }) => {
 const session = await getUser(req, res);
 const username = params?.username as string;
 const user = await getUserByUsername(username, "createdVersus", session?.user.id);
